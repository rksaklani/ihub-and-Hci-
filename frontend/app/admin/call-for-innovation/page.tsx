...
export default function CallForInnovationAdmin() {
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<CallForInnovation | null>(null)
  const [formData, setFormData] = useState<CallForInnovation>({
    title: '',
    description: '',
    deadline: '',
    link: '',
  ...
  const { data, isLoading, error, refetch } = useGetCallForInnovationsQuery()
  const [createItem, { isLoading: isCreating }] = useCreateCallForInnovationMutation()
  const [updateItem, { isLoading: isUpdating }] = useUpdateCallForInnovationMutation()
  const [deleteItem] = useDeleteCallForInnovationMutation()
  ...
  const handleEdit = (item: CallForInnovation) => {
    setEditingItem(item)
    setFormData(item)
    setIsModalOpen(true)
  }
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Call for Innovation</h1>
        <p className="mt-2 text-sm text-gray-600">Manage innovation calls</p>
      </div>

      <div className="glass-strong rounded-xl p-6 border-2 border-primary/10">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-gray-900">Innovation Calls</h2>
          <button
        ...
        {error ? (
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p>No innovation calls added yet</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((item: CallForInnovation) => (
              <div key={item._id} className="p-4 border border-gray-200 rounded-lg hover:shadow-lg transition-shadow">
                {item.image && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 flex items-center justify-between px-6 py-4 bg-white border-b border-gray-200 rounded-t-2xl">
              <h2 className="text-2xl font-bold text-gray-900">
                {editingItem ? 'Edit Innovation Call' : 'Add Innovation Call'}
              </h2>
              <button
